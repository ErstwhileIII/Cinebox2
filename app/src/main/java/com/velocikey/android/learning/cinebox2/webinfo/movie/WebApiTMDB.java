...
import com.velocikey.android.learning.cinebox2.webinfo.WebApi;
import com.velocikey.android.learning.cinebox2.webinfo.WebApiKeys;
...
//            Log.v(LOG_TAB, " initial info |" + rawJson.substring(0,Math.min(rawJson.length(),10)));
        ...
        //TODO determine whether to make the web access and the database access work together
        // put results into the database and get related videos and reviews;
        return results;
    }

    public ArrayList<MovieVideoInfo> getMovieVideos(int movieId) {
        //TODO change to pull video info for all movies in movieinfo table
        URL url;
...
//            Log.v(LOG_TAB, " initial info |" + rawJson.substring(0,Math.min(rawJson.length(),10)));
        ...
        try {
            ...
            movieId = videoList.getInt(TMDB_VIDEO_MOVIEID);

            // Now add info for each video
            for (int i = 0; i < videos.length(); i++) {
                JSONObject rawMovieInfo = videos.getJSONObject(i);

                int id = rawMovieInfo.getInt(TMDB_VIDEO_ID);
                ...
                results.add(new MovieVideoInfo(id, movieId, language, key, name, site, size, type));
            }
        } catch (JSONException e) {
            Log.e(LOG_TAB, "JSON parsing error", e);
            return null;
